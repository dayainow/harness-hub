...
class ToolService {
  ...
  async getTools(params?: GetToolsParams): Promise<{ success: boolean; data?: { success: boolean, data: Product[] }; error?: string }> {
    const res = await apiService.callWithErrorHandling<{ success: boolean, data: Product[] }>(
   ...
   * 3. 도구 승인 처리
   */
  async approveTool(id: string): Promise<{ success: boolean; data?: unknown; error?: string }> {
   ...
   * 4. 도구 반려 처리
   */
  async rejectTool(id: string): Promise<{ success: boolean; data?: unknown; error?: string }> {
    const res = await apiService.callWithErrorHandling(
